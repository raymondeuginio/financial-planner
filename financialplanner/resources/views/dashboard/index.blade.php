...
<div class="space-y-8">
    <section class="grid gap-4 sm:grid-cols-3">
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 class="text-sm font-medium text-slate-500">Income ({{ now()->isoFormat('MMM YYYY') }})</h3>
            <p class="mt-3 text-2xl font-semibold text-emerald-600">@idr($totalIncome)</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 class="text-sm font-medium text-slate-500">Expenses ({{ now()->isoFormat('MMM YYYY') }})</h3>
            <p class="mt-3 text-2xl font-semibold text-rose-600">@idr($totalExpense)</p>
        </div>
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 class="text-sm font-medium text-slate-500">Net Result</h3>
    ...
    <section class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-slate-800">Income vs Expense</h2>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-slate-800">Current Month Expense by Category</h2>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 class="text-lg font-semibold text-slate-800">Wallet Balances</h2>
                        ...
                        <li class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500 capitalize">{{ $wallet->type }}</p>
                                </div>
                            </div>
                ...
                <ul class="mt-4 space-y-4">
                    @forelse ($recentTransactions as $transaction)
                        <li class="flex items-start justify-between gap-3">
                            <div>
                                <p class="text-sm font-medium text-slate-700">{{ $transaction->description ?? 'No description' }}</p>
                                <p class="text-xs text-slate-500">{{ $transaction->occurred_at->isoFormat('MMM D, YYYY') }} • {{ $transaction->category?->name }} • {{ $transaction->wallet?->name }}</p>
                            </div>
                    ...
                    @empty
                        <li class="rounded-xl border border-dashed border-slate-200 p-4 text-center text-sm text-slate-500">
                            No transactions yet. Start by logging your first one.
                        </li>
                    @endforelse
                </ul>
            </div>
...
<script>
    ...
    const expenseLabels = @json($expenseCategories->map(fn ($row) => $row->category?->name ?? 'Uncategorised')->toArray());
