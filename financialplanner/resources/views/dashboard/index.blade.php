        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-slate-800">Perbandingan Pemasukan &amp; Pengeluaran</h2>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    ...
                    <p class="text-sm text-slate-500">Kategori tertinggi</p>
                </div>
                ...
                <ul class="mt-4 space-y-4">
                        ...
                        <li class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    ][$wallet->type] ?? ucfirst($wallet->type) }}</p>
                                </div>
                            </div>
                    ...
                    @empty
                        <li class="rounded-xl border border-dashed border-slate-200 p-4 text-center text-sm text-slate-500">
                            Tambahkan dompet untuk mulai memantau saldo.
                        </li>
                    @endforelse
                </ul>
            </div>
                ...
                <ul class="mt-4 space-y-4">
                    @forelse ($recentTransactions as $transaction)
                        <li class="flex items-start justify-between gap-3">
                            <div>
                                <p class="text-sm font-medium text-slate-700">{{ $transaction->description ?? 'Tanpa deskripsi' }}</p>
                                <p class="text-xs text-slate-500">{{ $transaction->occurred_at->isoFormat('DD MMM YYYY') }} • {{ $transaction->category?->name }} • {{ $transaction->wallet?->name }}</p>
                            </div>
...
<script>
    ...
    if (othersTotal > 0) {
        expenseLabels.push('Kategori Lainnya');
