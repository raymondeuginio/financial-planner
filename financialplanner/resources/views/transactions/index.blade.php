...
<div class="space-y-8">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500">Saring, tinjau, dan ekspor riwayat pemasukan serta pengeluaran.</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('transactions.create') }}" class="inline-flex items-center rounded-full bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-slate-700">Catat Transaksi</a>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($transactions as $transaction)
                    <tr class="hover:bg-slate-50/50">
                        <td class="px-4 py-3 text-slate-600">{{ $transaction->occurred_at->isoFormat('D MMM YYYY') }}</td>
                        <td class="px-4 py-3">
                            <div class="font-medium text-slate-800">{{ $transaction->description ?? 'Tanpa keterangan' }}</div>
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">{{ $transaction->wallet?->name ?? 'Tidak ada dompet' }}</span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">Transaksi tidak ditemukan. Coba ubah filter atau tambahkan transaksi baru.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
