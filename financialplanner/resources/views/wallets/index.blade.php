...
<div class="grid gap-8 lg:grid-cols-[1fr,1.5fr]">
    <div class="space-y-4">
        <div>
            <h1 class="text-2xl font-semibold text-slate-800">Dompet</h1>
            <p class="text-sm text-slate-500">Kelola sumber dana seperti kas, rekening bank, maupun dompet digital.</p>
        </div>
        <form action="{{ route('wallets.store') }}" method="POST" class="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-xl border-slate-200 text-sm focus:border-slate-500 focus:ring-slate-500" placeholder="Contoh: Dompet Tunai" required />
                @error('name')
            ...
            <div class="space-y-2">
                ...
                <select name="type" class="w-full rounded-xl border-slate-200 text-sm focus:border-slate-500 focus:ring-slate-500">
                    @foreach (['cash' => 'Tunai', 'bank' => 'Bank', 'ewallet' => 'Dompet Digital'] as $value => $label)
    ...
    <div class="space-y-4">
        <h2 class="text-lg font-semibold text-slate-800">Daftar Dompet</h2>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($wallets as $item)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-slate-600">{{ ['cash' => 'Tunai', 'bank' => 'Bank', 'ewallet' => 'Dompet Digital'][$item->type] ?? $item->type }}</td>
                            <td class="px-4 py-3 text-right font-medium text-emerald-600">@idr($item->income_sum)</td>
                            <td class="px-4 py-3 text-right font-medium text-rose-600">@idr($item->expense_sum)</td>
                            <td class="px-4 py-3 text-right font-semibold text-slate-800">@idr($item->net_flow)</td>
                            <td class="px-4 py-3">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-10 text-center text-sm text-slate-500">Belum ada dompet. Tambahkan dompet untuk mulai mencatat arus kas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
