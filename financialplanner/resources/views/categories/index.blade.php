...
<div class="grid gap-8 lg:grid-cols-[1fr,1.5fr]">
    <div class="space-y-4">
        <div>
            ...
            <p class="text-sm text-slate-500">Kelompokkan pemasukan dan pengeluaran dengan kategori berwarna.</p>
        </div>
        <form action="{{ route('categories.store') }}" method="POST" class="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="w-full rounded-xl border-slate-200 text-sm focus:border-slate-500 focus:ring-slate-500" placeholder="Contoh: Makan" required />
                @error('name')
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($categories as $item)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-slate-600">{{ ['income' => 'Pemasukan', 'expense' => 'Pengeluaran'][$item->type] ?? $item->type }}</td>
                            <td class="px-4 py-3">
